...
> \[!IMPORTANT] **Never Forget** to Close the File `file.close()` ensures to properly close the file after reading the contents in the file, as it helps not to waste any resources.
>
...
Pandas offer 2 primary data structures: 1. **DataFrame**: The 2-dimensional and size-mutable data structure with labelled axes _rows & columns_. 2. **Series**: The 1-dimensional labelled array _a single column or row_.

**Data Import and Export**: Pandas makes easy to read data from various sources _SQL databases, spreadsheets, ..._ and export them in these format.
...
_csv: comma separated file, df: data frame_
...
* **sort\_values(), sort\_index()**: Sort the Series by values or index labels.
...
A Python library that is commonly used for working with arrays, linear algebra, and matrices. _Numerical Python_: an open-source project. The array object in **Numpy** is called **ndarray**, it provides lots of supportive functions to easily work with ndarray.
...
matrix multiplication is _dot operation_ `numpy.dot()`
